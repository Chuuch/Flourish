...
const acceptUrl = `${env.API_URL}/auth/accept-invite`;

function renderAt(path: string) {
  return renderWithProviders(
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        <Route path={paths.acceptInvite} element={<AcceptInviteForm />} />
...
describe('AcceptInviteForm', () => {
  it('shows an error when the token is missing', () => {
    renderAt(paths.acceptInvite);

    expect(screen.getByRole('alert')).toHaveTextContent('This invite link is missing a token.');
  ...
  it('shows a validation error without calling the API', async () => {
    ...
    renderAt(`${paths.acceptInvite}?token=invite-token`);
  ...
  it('accepts the invite and shows sign in links', async () => {
    const user = userEvent.setup();

    server.use(
      mswHttp.post(acceptUrl, async ({ request }) => {
    ...
    expect(
      await screen.findByText('Invite accepted. Sign in with the password you just set.'),
    ).toBeInTheDocument();
  ...
  it('shows the server error message', async () => {
    ...
    server.use(
      mswHttp.post(acceptUrl, () =>
        HttpResponse.json({ error: { message: 'invite expired' } }, { status: 400 }),
      ),
    );

    renderAt(`${paths.acceptInvite}?token=expired`);
